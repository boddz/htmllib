...
class Lexer:
    ...
    def __eat_chars(self, until: list=[" ", ">", "=", "<", "/"], *, quotes=True) -> None:
        """
        **For internal use only**
        ...
        :param call: A callable object to call each iteration whilst ``until`` has not been hit, defaults to None
        :type call: Callable, optional

        :param quotes: A flag used to eat quotes and ignore ``until`` characters specified if prev is a quote token.
        :type quotes: optional, bool 
        ...
        if quotes:
            try:
                if self.tokens[-1].type == TokenTypes.QUOTE and self.index < len(self.stream): until=["\"", "\n"]
            except IndexError: print(f"{self}.__eat_chars method : No previous token, not inside of quotes, skipping")
    ...
    def lex(self, *, debug: bool=False) -> list(Token):
        ...
        while self.index < len(self.stream):
            ...
            if char not in ["<", ">", "\n"] and self.__inside_angle_bracket is False: self.cursor.increment()
                ...
                self.tokens.append(Token(TokenTypes.QUOTE, "\"", cursor))
                self.cursor.increment()
